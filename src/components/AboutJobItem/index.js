...
class AbotJobItem extends Component {
  state = {
    jobDataDetails: [],
    SimilarJobsData: [],
    ...
    if (responseJobData.ok === true) {
      ...
      const updatedJobDetailsData = [fetchedJobData.job_details].map(
        eachItem => ({
          ...
          skills: eachItem.skills.map(_eachSkill => ({
    ...
    if (jobDataDetails.length > 1) {
      ...
      return (
        <>
          ...
          <ul className="similar_jobs-ul-container">
